...
NOTES:
  ...
  - We need to fork a process otherwise, the Exec command replaces our current one
...
int main(void){
  ...
  int running = 1;
  int status = 0;; //Status of our child
  ...
  re = regcomp(&regexp, "[^echo${}!#()<'\\,[:digit:][:blank:]]", REG_EXTENDED);
  ...
  while (running == 1){
	...
	printf("Sources on the challenge Page \nAlso https://www.shellscript.sh/quickref.html \n");
      }
